...
for itr in range(epochs):
    ...
    c = c - lr * D_c
    iter_count += 1
    if iter_count > 999 or all(0.001 > l_val > -0.001 for l_val in loss):
        break

#######################################
# for itr in range(epochs):
#     for j, m in enumerate(a):
#         D_m = pre_d * sum([(y - _calc_val(a, c, X_pre[i])*a[j]) for i, y in enumerate(Y)])
#         a[j] = m - lr * D_m
#         loss[j] = D_m
#     D_c = pre_d * sum([y - _calc_val(a, c, X_pre[i]) for i, y in enumerate(Y)])
#     c = c - lr * D_c
...
#         break
# for itr in range(epochs):
#     Y_pred = [y - _calc_val(a, c, X_pre[i]) for i, y in enumerate(Y)]
#     for i, pred in enumerate(Y_pred):
#         for j, p_n in enumerate(a):
#             D_m = pre_d * pred * X_pre[i][j]
#             a[j] = p_n - (lr * D_m)
#             loss[j] = D_m
#     D_c = pre_d * sum([y - _calc_val(a, c, X_pre[i]) for i, y in enumerate(Y)])
#     c = c - lr * D_c
